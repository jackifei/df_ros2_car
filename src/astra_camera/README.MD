# ros2_astra_camera
来自官方文档：https://github.com/orbbec/ros2_astra_camera
-
...
```bash
cd ~/ros2_ws/src/astra_camera/astra_camera/scripts
...
常见问题
● Rviz2 上不显示点云或图像
发送消息的默认 QoS 是 sensor_data，即可靠性策略 = Best Effort（尽力而为）。请检查 Rviz2 上的可靠性策略是否也设置为 Best Effort，同时通过 ros2 topic info -v /xxx/topic 检查消息的 QoS 设置是否有问题。
● 多个摄像头无画面
可能是供电不足，请勿将所有摄像头连接到同一个集线器上，建议使用带独立供电的集线器。
也可能是分辨率过高，尝试降低分辨率。
如果彩色摄像头是 UVC 摄像头，可能是 UVC 的序列号未设置。
